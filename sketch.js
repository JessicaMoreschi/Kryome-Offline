...
<div class="cardPanel" style="display:flex; top:16px">
  ...
  // alert layout
  const alertHTML = ({ titleDisp, title, imageDisp, image, descriptionDisp, description, closeBtnDisp, margin = "50%" }) => `
<div class="cardPanel" style="display:flex; margin-top: calc(${margin})">
  ...
  <p style="display:${descriptionDisp}">${description}</p>
  <div class="cardPanelActions">
    <button class="roundedTag" id="closeBtnId" style="display: ${closeBtnDisp}" onclick="hideMyCard()">
      <img class="labelIcon" src="./assets/close.png">
      <h3>Close</h3>
    </button>
  </div>
...
<div class="torchPanel" style="display:${torchDisp}">
    ...
    //alain's dome
      ...
      let lev = document.getElementsByClassName('lev'); for (var levN = 0; levN < lev.length; levN++) {
        if (levN <= currentLevel) {
          lev[levN].removeEventListener("click", showLevelError);
        }
      }
  ...
  function hideMyCard(n) {
    ...
    //every time you close a panel reset testonline
    testonline = online;
  }
